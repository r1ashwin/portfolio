...
export default function HubIcon({
  ...
  return (
    <Link
      ...
      className="group absolute top-1/2 left-1/2 z-10 flex flex-col items-center gap-1.5 md:gap-2"
      style={{
        transform: `translate(calc(-50% + ${x}px), calc(-50% + ${y}px))`,
      }}
    >
      <div
        className={`flex h-11 w-11 items-center justify-center rounded-full border-2 transition-all duration-300 md:h-14 md:w-14 ${ringClass} group-hover:border-red-500 group-hover:shadow-[0_0_22px_rgba(239,68,68,0.4)]`}
      >
        <Icon
      ...
      <span
        className={`inline-block max-w-22 text-center text-[10px] leading-tight transition-all duration-300 md:max-w-26 md:text-xs ${
          pulseActive
